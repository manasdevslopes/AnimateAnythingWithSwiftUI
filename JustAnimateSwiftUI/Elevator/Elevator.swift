...
struct Elevator: View {
    ...
    func playChimeSound() {
        chimesSoundTimer = Timer.scheduledTimer(withTimeInterval: 5.5, repeats: false) { _ in
            playElevatorSound(sound: "elevatorChime", type: "m4a")
        }
    }
    ...
    func playDoorOpenCloseSound(interval: TimeInterval) {
        doorsOpenCloseSoundTimer = Timer.scheduledTimer(withTimeInterval: interval, repeats: false) { _ in
            playElevatorSound(sound: "doorsOpenClose", type: "m4a")
        }
    }
...
struct Elevator_Previews: PreviewProvider {
    static var previews: some View {
        Elevator().previewDevice("iPhone 12").previewDisplayName("iPhone 12")
//        Elevator().previewDevice("iPad Pro (9.7-inch)").previewDisplayName("iPad Pro (9.7 inch)")
    }
}
